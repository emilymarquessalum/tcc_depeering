...
def plot_list_as_line_plot(data_list, y=None, title='Data Line Plot', xlabel='Index', ylabel='Value', positive_color='green', negative_color='red',subfolder=None, max_labels=None, 
                           notes=None,
                           annotations=None, use_fill=True):
...
def plot_list_as_bar_plot(data_list, y=None, title='Data Bar Plot', xlabel='Index', ylabel='Value', subfolder=None, max_x_value=None, max_labels=None, use_colors=False,
    ...
    if do_top_n and y is not None:
        ...
        top_n_indices = sorted(range(len(y)), key=lambda i: y[i], reverse=True)[:do_top_n]
        data_list = [data_list[i] for i in top_n_indices]
        y = [y[i] for i in top_n_indices]
        colors = [colors[i] for i in top_n_indices]

    if sort_by_size and y is not None:
        # Sort data_list, y, and colors by y values
    ...
    def get_adjusted_separation_position(moment, separations):
        """Get the adjusted x-position for a separation line."""
        offset = 0
        for sep_moment in separations:
            if moment > sep_moment:
                offset += 1
            else:
                break
        return moment + offset - 0.5 
    ...
    if y is not None:
        ...
        if moments_of_separation:
            ...
            if max_labels and len(data_list) > max_labels:
                ...
                tick_labels = [data_list[i] for i in range(0, len(data_list), step)]
    ...
    if moments_of_separation:
        for moment in moments_of_separation:
            adjusted_pos = get_adjusted_separation_position(moment, moments_of_separation)
            plt.axvline(x=adjusted_pos, color='gray', linestyle='--', linewidth=1)
...
def plot_stacked_line_plot(data_lists, labels, x_labels=None, title='Stacked Line Plot', xlabel='Index', ylabel='Value', colors=None, subfolder=None, annotations=None, notes=None, max_labels=None, sort_by_size=True):
    ...
    if colors is None:
        colors = get_colors()
        #colors = colors[:max(6,len(data_lists[0]))]  # Use only as many colors as needed
