...
from src.ripe_bviews.read_bgpdump import BGPDumpSnapshotStats, read_bgpdump
...
def load_bview_data_timeline(start_date, end_date, asn_and_prefix, rrc, day_delta=None, time_delta_hours=None, time_str=None, ip_version=None, skip_if_missing=0, ignored_dates=None) -> tuple[list[BGPDumpSnapshotStats], list]:
    ...
    while current_date < end_date: 
        ...
        output_file_name = get_rib_output_file_name(rrc, current_date.strftime("%Y%m%d"), time_str, asn_and_prefix[0], asn_and_prefix[1])
        stats = read_bgpdump(output_file_name, asn_and_prefix[0], rrc, ip_version, monitor_prefix=asn_and_prefix[1], date=current_date.strftime("%Y%m%d"), time=current_date_str, skip_if_missing=(skip_if_missing > 0))
