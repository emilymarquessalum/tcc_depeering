...
def plot_new_routes_over_time_by_member_that_added_it_optimized(all_stats, labels_summarized, name, ip_version, subfolder, max_labels):
    ...
    # 1. Single Pass: Extract time-series arrays and grand totals simultaneously
    # Each member will map to a list of size `num_stats` pre-filled with 0
    ...
    if not all_members:
        print("No members found with new routes, skipping the plots.")
        return

    # 2. Ultra-fast Sorting: Sorting keys are now cheap O(1) dictionary lookups
    ...
    # 3. Build time-series matrix for Top N
    new_routes_list_of_lists_top_n = [member_series_map[m] for m in top_n_members]
    plot_labels_lines = list(top_n_members)

    # 4. Optimized "Other" logic: Vectorized sum across pre-calculated arrays
        ...
        for m, series in member_series_map.items():
            if m not in top_n_set:
                for idx in range(num_stats):
                    other_series[idx] += series[idx]
        
        new_routes_list_of_lists_top_n.append(other_series)
        plot_labels_lines.append("Other")

    # 5. Handle potential mismatches between labels and data length
    labels = labels_summarized[:len(new_routes_list_of_lists_top_n[0])] if labels_summarized else []

    # 6. Generate the Stacked Line Plot
    plot_stacked_line_plot(
        new_routes_list_of_lists_top_n,
        plot_labels_lines,
        x_labels=labels,
        title=f'New Routes Over Time By Member That Added It - {name} - IP{ip_version}',
        xlabel='Time',
        ylabel='Number of New Routes',
        subfolder=subfolder,
        max_labels=max_labels
    )

    # 7 & 8. Build aggregated data for Bar Plot (Instant O(1) using pre-calculated totals)
    sum_of_new_routes_per_member = [member_totals[m] for m in top_n_members]
    plot_labels_bars = list(top_n_members)

    if show_rest_as_single_group and len(all_members) > top_n_number:
        # Sum totals of all members not in the top N
        total_other = sum(total for m, total in member_totals.items() if m not in top_n_set)
        sum_of_new_routes_per_member.append(total_other)
        plot_labels_bars.append("Other")

    # Generate the Bar Plot
    plot_list_as_bar_plot(
        plot_labels_bars,
        sum_of_new_routes_per_member,
        title=f'Sum of New Routes By Member That Added It - Top {top_n_number} - {name} - IP{ip_version}',
        xlabel='Members / Groups',
        ylabel='Sum of Number of New Routes',
        subfolder=subfolder, 
        max_labels=max_labels
    )
...
def calculate_routes_over_time(all_stats: list[BGPDumpSnapshotStats], group_by_path_length=False,
    ...
    # Changed to a dict tracking lists per member to avoid append errors
    routes_over_time_by_member = {} 

    for stat in all_stats:
        total_routes_by_length = []  
        total_routes = 0
        ...
        if group_by_path_length:
            for member_mappings in stat.mappings.values():
                for mapping in member_mappings:
                    ...
                    total_routes_by_length[path_length] += 1
    ...
    if group_by_member:
        return routes_over_time_by_member
...
def plot_routes_over_time(all_stats, labels_summarized, title_start, title_end, subfolder, max_labels):
    ...
    # --- FIXED BAR PLOT LOGIC ---
    route_as_path_length_distribution_in_last_snapshot = routes_over_time_by_length[-1]
    
    # Grab data from path length 1 onwards
    plot_data = route_as_path_length_distribution_in_last_snapshot[1:] 
    # Ensure X-axis labels perfectly match the length of plot_data
    x_labels = [i for i in range(1, len(plot_data) + 1)] 
...
def bview_timeline_routes(all_required_data):
    ...
    number_of_routes_lost_that_did_not_remove_members_or_reachables_per_member, number_of_routes_added_that_did_not_add_members_or_reachables_per_member, total_losses_that_made_shortest_path_longer_per_snapshot = plot_mapping_changes(all_stats, labels_summarized, name, ip_version, subfolder, max_labels)
    plot_routes_over_time(all_stats, labels_summarized, title_start, title_end, subfolder, max_labels)
    
    plot_new_routes_over_time_by_member_that_added_it(all_stats, labels_summarized, name, ip_version, subfolder, max_labels)
