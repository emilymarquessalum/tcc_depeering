...
class ReachableLeftAndCameBackASPathInfo:
    ...
    def plot_came_back_length_changes(self, title_start, title_end, subfolder):
        ...
        plot_map_as_bar_plot(
            length_changes,
            title=title_start + "AS Path Length Changes (Best-Path) for Oscillating Reachables with " + self.category_name + title_end,
            ylabel="Count of Reachables", 
...
def get_came_back_through_count_categories(category_name, matched_reachables) -> ReachableLeftAndCameBackASPathInfo:
    ...
    for r in matched_reachables:
        previous_best_path_length = r.get("previous_best_path_length")
        current_best_path_length = r.get("current_best_path_length")
        if previous_best_path_length is not None and current_best_path_length is not None:
            if current_best_path_length == previous_best_path_length:
                came_back_with_same_length.append(r)
                if r["previous_best_path"] == r["current_best_path"]:
                    came_back_with_same_path.append(r) 
                continue
            came_back_linked_to_path_length_change.append(
                {
                    "reachable": r,
                    "previous_best_path_length": previous_best_path_length,
                    "current_best_path_length": current_best_path_length,
                    "change": previous_best_path_length - current_best_path_length,
                    "previous_best_path": r.get("previous_best_path"),
                    "current_best_path": r.get("current_best_path")
                }
            )
            if current_best_path_length < previous_best_path_length:
...
def get_came_back_metrics_from_reachables_that_left_and_came_back(reachable_oscillation_metrics, all_stats, remove_prepend=True) -> dict:
    ...
    for reachable_asn, osc_info in reachable_oscillation_metrics.oscillation_info.items():
        ...
        reachables_left_and_came_back[reachable_asn] = { 
            ...
            "previous_best_path_length": previous_stat.get_best_as_path_length_for_reachable(reachable_asn, remove_prepend)[0],
            "current_best_path_length": current_stat.get_best_as_path_length_for_reachable(reachable_asn, remove_prepend)[0],
            "current_best_path": current_stat.get_best_as_path_length_for_reachable(reachable_asn, remove_prepend)[1],
            "previous_best_path": previous_stat.get_best_as_path_length_for_reachable(reachable_asn, remove_prepend)[1]
        }
