...
def check_asn_connection(all_required_info, asn_to_consider=None):

    all_stats, _, _ = all_required_info.get("timeline")
    
    if asn_to_consider is None:
        asn_to_consider = input("Enter ASN to check connectivity metrics for: ")

    presence_list = get_asn_presence_list(all_stats, asn_to_consider)

    is_present = any(presence_list)
    if not is_present:
        print(f"AS {asn_to_consider} is not present in any snapshot of the timeline.")
        return
    print(f"AS {asn_to_consider} presence over time: {presence_list}")
    check_asn_connectivity_metrics(asn_to_consider, all_stats)


def check_asn_connection_for_relevant_ases(all_required_info):
    
    for icp in relevant_icps:
        asn_to_consider = icp["asn"]
        print(f"Checking connectivity metrics for AS {asn_to_consider} ({icp['name']})...")
        check_asn_connection(all_required_info, asn_to_consider)
