...
class BGPDumpSnapshotStats:
    ...
    def get_prefix_mappings(self):
        ...
        for member, reachables in self.mappings.items():
            # Pre-convert member to string once per outer loop iteration
            member_str = str(member)
            
            for mapping in reachables:
                # Use standard dict lookup; handle missing key gracefully
                ...
                # Avoid redundant string conversions inside the inner loop
                if member_str == str(reachable): 
    ...
    def get_unique_prefixes_for_member(self, member_asn: str | int) -> set[str]:
       # Ensure type consistency up front
        member_asn_str = str(member_asn)
         
        # 1. Directly fetch only this member's reachables (O(1) lookup)
        reachables = self.mappings.get(member_asn_str, [])
        if not reachables:
            return set()

        unique_prefixes = set()
        
        # 2. Only loop through this specific member's mappings
        for mapping in reachables:
            prefix = mapping.get("prefix")
...
def _get_stats_filename(rrc_option, date, time,ip_version, origin_asn=None):
    if origin_asn is not None:
        return f"cache/{rrc_option}/{ip_version}/{origin_asn}/bview_cache.{date}.{time}.json"
    return f"cache/{rrc_option}/{ip_version}/bview_cache.{date}.{time}.json"

def get_stats_filenames(monitor_as, monitor_prefix, date, time, rrc, ip_version, origin_asn=None):
    ...
    for rrc_option in rrc_options: 
        file_names.extend(append_roots(_get_stats_filename(rrc_option, date, time, ip_version, origin_asn)))
    return file_names
    #return append_root(f"cache/bview_cache.{date}.{time}_{monitor_as}_{monitor_prefix}.json")


def read_bgpdump_from_file_options(monitor_as, rrc, ip_version, 
                                   origin_asn, 
                                   monitor_prefix=None, date=None, time=None,
                                   skip_if_missing=0) -> list[BGPDumpSnapshotStats]:
    
    print("[DEBUG] Attempting to read bgpdump")
    
    stats = read_bgpdump(monitor_as, rrc, ip_version, monitor_prefix, date, time,
                         origin_asn,
...
def read_bgpdump(monitor_as, rrc, ip_version, monitor_prefix=None, date=None, time=None, origin_asn=None, skip_if_missing=0) -> BGPDumpSnapshotStats:
    stats = BGPDumpSnapshotStats(monitor_as, monitor_prefix, date, time)
    #details_file = f"data/stats_{file1}_{monitor_as}_{monitor_prefix}.json"
    details_files = get_stats_filenames(monitor_as, monitor_prefix, date, time, rrc, ip_version, origin_asn)
