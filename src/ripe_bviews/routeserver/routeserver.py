...
if __name__ == "__main__":
    ...
    print(f"ASNs that lost more than 10% between i-2 and i-1 (total: {len(asns_that_lost_more_than_ten_percent_between_i_minus_two_and_i_minus_one)}):")
    for asn, percentage_loss, routes_i_minus_two, routes_i_minus_one in asns_that_lost_more_than_ten_percent_between_i_minus_two_and_i_minus_one:
    ...
    for date_index in range((end_time - start_time).days + 1):
        total_routes_for_date = sum(asn_to_routes_map[asn][date_index][1] for asn in asn_to_routes_map)
        total_routes_over_time.append(total_routes_for_date)

    plot_list_as_line_plot(total_routes_over_time, 
                           y=[date for date, _ in asn_to_routes_map[next(iter(asn_to_routes_map))]],
    ...
    for asn, change_percentage in sorted(most_changed_consistent_asns, key=lambda x: -abs(x[1]))[:5]:
    ...
    for asn, change_percentage in sorted(consistent_asns_with_most_straightforward_change, key=lambda x: -abs(x[1]))[:5]:
