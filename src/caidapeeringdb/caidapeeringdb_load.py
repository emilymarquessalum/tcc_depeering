...
def download_peeringdb_dump(date):
    ...
    output_folder = "/home/emily/Desktop/projects/furg/tcc_depeering_elixir/data/caida-peeringdb/"
...
def get_ases_by_number_of_lost_peered_connections(all_files, base_path="/home/emily/Desktop/projects/furg/tcc_depeering_elixir/data/caida-peeringdb/",
...
def load_connections_over_time_for_asns(all_files, asns_to_search, connections_should_be="all"):
    # Normalize asns_to_search to a set of IDs for O(1) lookup
    ...
    for file in all_files: 
        ...
        # each net has:
        # ix_id, is_rs_peer, updated
        for net in data.get("netixlan", {}).get("data", []):
            asn = get_asn_from_net(net)
            if asn in search_ids:
                indexed_data[asn].append(net)

        # Now just process the ASNs we care about
        for asn in search_ids:
            asn_conns = indexed_data.get(asn, [])
...
def process_connections(ixp_connections, connections_should_be, one_connection_per_ix=True) -> list[dict]: 
    ...
    # 1. Filter by peering status (if necessary)
    if connections_should_be != "all":
        target_status = (connections_should_be == "peered")
        ixp_connections = [c for c in ixp_connections if c.get("is_rs_peer") == target_status]

    if one_connection_per_ix:
        # 2. Group by ix_id and find the latest 'updated' in one pass
        # Using a dict to track the 'best' connection seen so far for each IX
        latest_per_ix = {}
        for conn in ixp_connections:
            ix_id = conn.get("ix_id")
            if not ix_id: continue
            
            if ix_id not in latest_per_ix or conn["updated"] > latest_per_ix[ix_id]["updated"]:
