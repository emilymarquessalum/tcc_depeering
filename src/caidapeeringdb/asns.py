...
def format_asn_to_search(asn_to_search):
    """Format ASN to search for display purposes."""
    asn, name = asn_to_search[0], asn_to_search[1]
    return f"{name} (AS{asn})" if name else f"AS{asn}"
...
def plot_connections_for_asns_over_time(connections_over_time_by_asn_peered, connections_over_time_by_asn_not_peered, asns_to_search, all_files, ixp_by_continent_count=None):
    ...
    for asn, connections_over_time in connections_over_time_by_asn_peered.items():
        ...
        number_of_connections_peered = [len(connections) for _, connections in connections_over_time]
        #print(connections_over_time[0][1])
        ...
        plot_connections_gained_into_groups_over_time(connections_over_time, asn_to_search, subfolder, ixp_id_to_name)
        

        if plot_transitions:
            plot_connection_transitions(connections_over_time, connections_over_time_by_asn_not_peered, asn_to_search, subfolder, asn)
        ...
        plot_list_as_line_plot(
            ...
            y=dates_y,  
            subfolder=subfolder,
            title=f"Route Server IXP Connections for {format_asn_to_search(asn_to_search)} over time",
            xlabel="Date",
            max_labels=8,
        ...
        if len(sorted_continents) > 1:
            plot_stacked_line_plot(
                ...
                title=f"In Route Server Connections by Continent for {format_asn_to_search(asn_to_search)} over time",
                xlabel="Date",
                ylabel="Number of Connections",
                subfolder=subfolder
            )
        ...
        if plot_win_loss:
            plot_list_as_win_loss_bar_plot(
                ixps_over_time,
                f"In Route Server IXP Connections in Route Server for {format_asn_to_search(asn_to_search)} over time - Win vs Loss",
            ...
            plot_list_as_win_loss_bar_plot(
                ...
                f"Not In Route Server IXP Connections for {format_asn_to_search(asn_to_search)} over time - Win vs Loss",
                y=dates_y[1:],
                xlabel='Date',
                ylabel='Gain/Loss',
                subfolder=subfolder
        ...
        plot_stacked_win_loss_bar_plot_by_continent(
            ...
            title=f"Region-wise IXP Connection Gains vs Losses Over Time for {format_asn_to_search(asn_to_search)} - In Route Server",
            y=dates_y[1:],
            subfolder=subfolder,
            create_text_report=True
        )
        ...
        if plot_win_loss:
            ...
            plot_stacked_win_loss_bar_plot_by_continent(
                ...
                title=f"Region-wise IXP Connection Gains vs Losses Over Time for {format_asn_to_search(asn_to_search)} - Not In Route Server",
                y=dates_y[1:],
                subfolder=subfolder,
        ...
        if plot_ixps_lost:
            ...
            plot_list_as_bar_plot(
                ...
                title=f"Number of IXPs Lost by Continent for {format_asn_to_search(asn_to_search)} over time",
                xlabel="Continent",
        ...
        plot_list_as_line_plot(
            ...
            subfolder=subfolder,
            title=f"IXP Connections that were not in Route Server for {format_asn_to_search(asn_to_search)} over time",
            xlabel="Date",
        ...
        plot_stacked_line_plot(
            ...
            subfolder=subfolder,
            title=f"IXP Connections for {format_asn_to_search(asn_to_search)} over time (Route Server x Not-Route-Server)",
            #show_title=False,
...
def plot_connection_transitions(connections_over_time, connections_over_time_by_asn_not_peered, asn_to_search, subfolder, asn):
        transitions_from_one_type_to_another_over_time = []
...
def plot_connections_gained_into_groups_over_time(connections_over_time, asn_to_search, subfolder, ixp_id_to_name): 

        
        connections_peered = [set([c["ix_id"] for c in connections if c["is_rs_peer"]]) for _, connections in connections_over_time]
        connections_not_route_server = [set([c["ix_id"] for c in connections if not c["is_rs_peer"]]) for _, connections in connections_over_time]
        
        connections_gained_over_time = [connections_peered[i] - connections_peered[i - 1] for i in range(1, len(connections_peered))]

        connections_gained_that_existed_in_a_previous_snapshot_over_time = []
        connections_gained_that_did_not_exist_in_a_previous_snapshot_over_time = []
        connections_gained_that_existed_as_not_route_server_in_a_previous_snapshot_over_time = []


        for i in range(len(connections_gained_over_time)): 
            connections_gained = connections_gained_over_time[i]

            all_previous_connections = set() 

            all_previous_connections_not_route_server = set()
            
            for j in range(i + 1):
                all_previous_connections.update(connections_peered[j])

            for j in range(i + 1):
                all_previous_connections_not_route_server.update(connections_not_route_server[j])


            connections_gained_that_existed_as_not_route_server_in_a_previous_snapshot = connections_gained & all_previous_connections_not_route_server
            connections_gained_that_existed_as_not_route_server_in_a_previous_snapshot_over_time.append(len(connections_gained_that_existed_as_not_route_server_in_a_previous_snapshot))


            connections_gained_that_existed_in_a_previous_snapshot = (connections_gained & all_previous_connections) - connections_gained_that_existed_as_not_route_server_in_a_previous_snapshot
            
            ixp_names = [ixp_name_short_format(ixp_id_to_name.get(ixp_id, str(ixp_id))) for ixp_id in connections_gained_that_existed_in_a_previous_snapshot]
            print(ixp_names)
            connections_gained_that_existed_in_a_previous_snapshot_over_time.append(len(connections_gained_that_existed_in_a_previous_snapshot))

            connections_gained_that_did_not_exist_in_a_previous_snapshot = connections_gained - all_previous_connections
            connections_gained_that_did_not_exist_in_a_previous_snapshot_over_time.append(len(connections_gained_that_did_not_exist_in_a_previous_snapshot))



        plot_stacked_line_plot(
            [connections_gained_that_existed_in_a_previous_snapshot_over_time, connections_gained_that_did_not_exist_in_a_previous_snapshot_over_time, connections_gained_that_existed_as_not_route_server_in_a_previous_snapshot_over_time],
            ["Connections Gained that Existed in a Previous Snapshot", "Connections Gained that Did Not Exist in a Previous Snapshot", "Connections Gained that Existed as Not Route Server in a Previous Snapshot"],
            x_labels=[file_date for file_date, _ in connections_over_time[1:]],
            subfolder=subfolder,
            title=f"Connections Gained Over Time for ASN {asn_to_search[0]} ({asn_to_search[1]})",
            xlabel="Date",
            ylabel="Number of Connections Gained",
            max_labels=8,
            sort_by_size=False,
            rotate_labels=False
        )
...
def plot_asns_analysis(all_files, asns_to_search_list, ixp_by_continent_count):
    ...
    print("Loading connections over time for ASNs...")
    ...
    print("Calling plot_connections_for_asns_over_time")
